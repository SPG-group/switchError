...
void data::fixPhase() {
	vrb.title("Fixing phase");
	for (int i = 0 ; i < vec_names.size() ; i ++) {
		if (hap1[i].size() && gen1[i].size()) {
...
bool data::solveTrio(int locus, int cidx, int fidx, int midx) {
	...
	pknown[cidx][locus] = (phased && cg==1); pknown[fidx][locus] = (phased && fg==1); pknown[midx][locus] = (phased && mg==1);
	merrors[cidx][locus] = mendel;merrors[fidx][locus] = mendel;merrors[midx][locus] = mendel;
...
bool data::solveDuoFather(int locus, int cidx, int pidx) {
	...
	pknown[cidx][locus] = (phased && cg==1); pknown[pidx][locus] = (phased && pg==1);
	merrors[cidx][locus] = mendel;merrors[pidx][locus] = mendel;
	if (mendel) return true;
	else return false;
}
